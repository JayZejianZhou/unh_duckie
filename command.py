...
# ZZ -------
AGENT_BASE_IMG = f"duckietown/dt-duckiebot-interface:{BRANCH}"
...
class DTCommand(DTCommandAbs):
    ...
    def command(shell: DTShell, args):
        ...
        try:
            agent_base_image0 = AGENT_BASE_IMG
            # zz
        ...
        #agent_container_name = f"{prefix}agent"   # ZZ____________________________________________________
        agent_container_name = "drz-duckiebot-interface"
        bridge_container_name = f"{prefix}dt-duckiebot-fifos-bridge"
        
        dtslogger.info(f"???agent container is {agent_container_name}")
        ...
        if use_ros:
            ...
            # dtslogger.info("zz"+f"{AGENT_ROS_PORT}/tcp")
            ros_params = {
                "image": ros_image,
